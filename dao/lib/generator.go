...
func (g *Generator) ParserStruct(structs []interface{}) (ret *Generator) {
	for _, ptr := range structs {
		reType := reflect.TypeOf(ptr)
		if reType.Kind() != reflect.Ptr || reType.Elem().Kind() != reflect.Struct {
			panic("param doesn't struct")
		}
		var (
...
func (g *Generator) checkConfig() (err error) {
	if len(g.config.ImportPkgs) == 0 {
		err = errors.New("import package doesn't set")
		return
	}
	if len(g.config.PkgName) == 0 {
		err = errors.New("package name doesn't set")
		return
	}
