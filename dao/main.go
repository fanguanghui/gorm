...
func parseFlags() {
	...
	for _, v := range s {
		cnf.imports = append(cnf.imports, lib.ImportPkg{
			Pkg: v,
		})
	}
}
...
func main() {
	...
	p := lib.NewParser(cnf.input)

	gen := lib.NewGenerator(cnf.input).SetImportPkg(cnf.imports).SetLogName(logName)
	if transformErr {
